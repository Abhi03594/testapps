    ...
    cat > trust-policy.json << EOF
{
    ...
    "Statement": [{
        ...
        "Condition": {
            "StringEquals": {
                "${ISSUER_URL_WITHOUT_PROTOCOL}:sub": "system:serviceaccount:${NAMESPACE}:kafka"
            }
        }
    }]
}
...
applyServiceAccount(){
    ...
    kubectl create sa kafka --namespace ${NAMESPACE}
    kubectl annotate sa kafka eks.amazonaws.com/role-arn=$ROLE_ARN --namespace ${NAMESPACE}
    echo "Service Account Created: kafka"
}
